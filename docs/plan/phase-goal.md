...
- [ ] Defender destruction triggers a visual break effect (fade + scale-down or particle scatter) at the defender's position before entity removal [US-20]
- [ ] Placing a defender plays a bounce-in animation (scale tween from ~0.3 to ~1.1 overshoot to 1.0) [US-20]
- [ ] Projectile impact shows a small visual burst (expanding circle or particle puff) at the hit point [US-20]
- [ ] Camera shakes (1-2px amplitude, 200-500ms duration) when the final-wave announcement triggers [US-20]
...
- [ ] Furniture silhouette shapes (minimum 2 distinct pieces) are rendered along the top edge of the play area, at depth lower than gameplay entities [US-22]
- [ ] 3-5 decorative toy detail shapes are rendered on grid cells at level start, at depth lower than defenders and enemies [US-22]
- [ ] Floating dust mote particles (10-20 semi-transparent dots/circles) drift across the play area continuously during gameplay [US-22]
- [ ] TitleScene renders bedroom-themed background elements beyond a solid color fill [US-22]
- [ ] GameOverScene renders bedroom-themed background elements beyond a solid color fill [US-22]
- [ ] No atmosphere element has depth >= defender/enemy entity depth, and no atmosphere element receives pointer input that could block grid clicks [US-22]
